...
HEADERS_BASE = {
    ...
    "User-Agent": "enterprise-stale-branch-audit"
}
...
def calculate_cutoff(now_et, months):
    cutoff_month = now_et.month - months
    cutoff_year = now_et.year
    while cutoff_month <= 0:
        cutoff_month += 12
        cutoff_year -= 1
    return datetime.datetime(cutoff_year, cutoff_month, 1, tzinfo=ET)
...
def main():
    ...
    for repo in repos:
        ...
        while True:
            branches = github_get(
                ...
                headers,
                params={"per_page": 100, "page": branches_page}
            )
            if not branches:
                break
    ...
    with open(f"{args.out}/stale_report.txt", "w") as f:
        f.write(
            ...
            f"Inactive for ≥ {args.months} calendar months (US Eastern Time)\n\n"
        )
        for s in stale:
            f.write(
                f"Repo: {s[0]}\n"
...
<p>Inactive for <b>≥ {args.months} calendar months</b> (US Eastern Time).</p>
...
<ul>
  ...
  <li>Deletion must occur via a separately approved cleanup pipeline</li>
</ul>
""")
